...
namespace CSL.Mach1
{
    ...
    public class CRC
    {
        private static byte[] API_CrcTable = new byte[]{
        ...
        {
            byte crc = 0x00;
            for(int i=0; i<len;i++)crc = API_CrcTable[(crc ^ data[i]) & 0xFF];
        ...
        public static bool VerifyCRC(byte[] data)
        {
            try
            {
                byte crc = CalculateCRC(data, data.Length - 1);

                if (crc != data[data.Length - 1]) return false;
                return true;
            }
            catch
            {
